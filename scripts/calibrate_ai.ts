...
/**
 * AI CALIBRATION SCRAPER
 * 
 * This tool extracts the "Mechanical Patterns" from known AI outputs
 * to establish the AI decision boundary.
 */
...
function calculatePlanningEntropy(sentences: string[]): { entropy: number, clauseVar: number } {
    if (sentences.length < 3) return { entropy: 0, clauseVar: 0 };
...
async function calibrateAI() {
    ...
    const aiDist: Record<string, any> = {};

    for (const genre of genres) {
        const dir = path.join(process.cwd(), `data/ai/${genre}`);
    ...
    process.stdout.write(JSON.stringify(aiDist, null, 2));
}
