...
export default function Home() {
  ...
  // Auto-scan effect
  useEffect(() => {
    // If output exists, we scan the output. Otherwise, we scan the input.
    const targetText = outputText || inputText;
    ...
    const timer = setTimeout(() => {
      const result = calculateHumanScore(targetText);
  ...
  const handleHumanize = useCallback(async () => {
    ...
    try {
      ...
      if (data.text) {
        ...
        setHumanScore(parseFloat(data.humanScore));
      }
    } catch (error) {
